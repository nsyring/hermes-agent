...
def validate_config(config) -> bool:
    ...
    has_pw = bool(os.getenv("NEXTCLOUD_TALK_APP_PASSWORD", "").strip())
...
class NextcloudTalkAdapter(BasePlatformAdapter):
    ...
    async def _handle_command(self, text: str, chat_id: str) -> Optional[str]:
        ...
        if cmd in ("/new", "/reset"):
            ...
            try:
                ...
                logger.exception("nextcloud_talk: reset session failed")
    ...
    async def send(
        ...
        # Ack-edit: if a "Thinking..." ack is pending for this chat, edit it
        # into the response instead of sending a new message.
        ack = self._pending_ack
        if ack and ack["chat_id"] == chat_id and self._edit_ack_into_response:
            self._pending_ack = None
            ok, err = await self._client.edit_message(chat_id, ack["message_id"], content)
            if ok:
