...
class NextcloudFilesClient:
    ...
    async def mkdir(self, remote_path: str) -> bool:
        """Create a directory via WebDAV MKCOL, including missing ancestors.

        WebDAV semantics that shaped this:

        * MKCOL on an EXISTING collection returns **405** — that is success
          for our purposes (idempotent sync retries), not an error.
        * MKCOL with a MISSING parent returns **409** — walk the path and
          create each ancestor first, otherwise nested trees created in one
          burst (deep local mkdir -p + files) never make it to the server.
        """
        segments = [s for s in remote_path.strip("/").split("/") if s]
        partial = ""
        for segment in segments:
            partial = f"{partial}/{segment}"
            if not await self._mkcol_single(partial):
                return False
        return True

    async def _mkcol_single(self, remote_path: str) -> bool:
        """One MKCOL; 405 (already exists) counts as success."""
        url = self._dav_url(remote_path)
        try:
